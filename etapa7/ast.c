...
HASH_NODE *addStringName(AST *node);
...
void astPrint(AST *node, int level) {
	...
	switch(node->type){
		...
		case AST_ASS_VEC: fprintf(stderr, "AST_ASS_VEC"); break;
...
HASH_NODE *addStringName(AST *node){
	...
	if(!node->symbol)
		return 0;
	if((temp = hashFind(node->symbol->text)) != 0){
		temp = createName(temp);
	}
	return temp;
...
void addDataVarHash(AST *node){
	...
	{
	case AST_DECVAR:
		if ((!node->son[0]) && (!node->symbol) && (!node->son[1]))
			return;
		if((hashNode = hashFind(node->symbol->text)) !=0 )
			addData(hashNode, node->son[1]->symbol?node->son[1]->symbol->text:0);
		break;		
	}

	for(i = 0; i<MAX_SONS; i++)
		addDataVarHash(node->son[i]);
...
void astDescompila(AST *node){
	...
	switch(node->type){
		...
		case AST_ASS_VEC: fprintf(yyout, "%s[", node->symbol->text);
			astDescompila(node->son[0]);
			fprintf(yyout, "]");
			fprintf(yyout, " = ");
			astDescompila(node->son[1]);
